...
export default function ServicesGrid() {
  const services = [
    ...
    {
      id: "04",
      title: "Full-Stack Modernization & DevOps",
      hero: code1,
      thumb: code1,
    },
  ];
  ...
  const [heroSrc, setHeroSrc] = useState(services[0].hero);

  return (
    <div
      id="pdEngg"
      className="w-full bg-gradient-to-br from-[#7F3DFF] to-[#5C22FF] p-12 scroll-mt-[140px] text-white overflow-hidden"
    >
      {/* TOP HERO */}
        ...
        {services.map((service, index) => {
          ...
          return (
            <div
              ...
              onMouseLeave={() => {
                setHovered(0);
                setHeroSrc(services[0].hero);
              }}
              style={{
                width: isHovered ? expanded : collapsed,
                height: "210px",
              }}
            >
              {/* CARD BODY */}
              <div className="p-6 rounded-xl border border-black/30 bg-white h-full flex items-center justify-between relative z-10 overflow-hidden">
                ...
                <div className="flex flex-col justify-between h-full w-full">
                  <div>
                    <div className="text-2xl font-bold text-[#183058] mb-3">
                      {service.id}
                    </div>
                    <h3 className="text-lg w-40 font-medium leading-snug text-[#183058]">
                      {service.title}
                    </h3>
                  </div>
